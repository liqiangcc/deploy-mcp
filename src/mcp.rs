...
impl DeployMcp {
    ...
    #[tool(
        description = "Explicitly roll back a historical deployment only when its durable deployment-bound rollback reference is still active and the configured environment contract has not changed. The tool accepts only a deployment id; remote paths and task names are never caller-controlled."
    )]
    async fn rollback_deployment(
        &self,
        Parameters(args): Parameters<RollbackDeploymentArgs>,
    ) -> CallToolResult {
        match self
            .application
...
fn execution_result_json(result: &DeploymentExecutionResult) -> Value {
    ...
    if let Some(object) = value.as_object_mut() {
        object.insert(
            "rollback_reference_available".to_owned(),
            Value::Bool(result.rollback_reference_available),
        );
        object.insert(
...
mod tests {
    ...
    impl DeploymentApi for FakeApi {
        ...
        async fn rollback_deployment(&self, deployment_id: &str) -> AppResult<RollbackOutcome> {
            Err(AppError::new(
                ...
                format!("no rollback reference for {deployment_id}"),
            ))
        }
    }
    ...
    async fn rollback_tool_exposes_only_deployment_id() {
        let mcp = DeployMcp::new(Arc::new(FakeApi));
        let result = mcp
            .rollback_deployment(Parameters(RollbackDeploymentArgs {
